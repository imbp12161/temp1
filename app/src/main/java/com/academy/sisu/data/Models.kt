...
/** 한 구간의 수업 시수(한 사이클 등원 횟수). from 날짜부터 이 시수를 적용한다. (시수 변경 이력용) */
data class SessionSeg(
    val from: LocalDate,
    val sessions: Int
)
 ...
 * sessionPlan: 수업 시수 이력. from 오름차순. 가장 최근 항목이 현재 시수.
 *   시수를 바꾸면 '변경 적용일'부터 새 시수로 사이클을 세고, 그 전 기록은 유지된다.
...
data class Student(
    ...
    val sessionPlan: List<SessionSeg>,
    val start: LocalDate,
    val color: Long,
    val group: String = "",
    ...
    /** 현재(가장 최근) 수업 시수 — 표시·입력 기본값용 */
    val sessions: Int
        get() = (sessionPlan.maxByOrNull { it.from } ?: sessionPlan.firstOrNull())?.sessions ?: 12
}

/** 어떤 날짜의 한 학생 등원 정보. cycle: 0-based 사이클, idx: 사이클 내 1..n, n: 그 사이클의 목표 시수, end: 정산일 여부 */
data class DayInfo(val cycle: Int, val idx: Int, val n: Int, val end: Boolean)
