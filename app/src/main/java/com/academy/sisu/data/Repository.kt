...
class Repository(context: Context) {
    ...
    private fun studentsToJson(list: List<Student>): JSONArray {
        ...
        for (s in list) {
            ...
            o.put("schedules", sch)
            val sp = JSONArray()
            s.sessionPlan.forEach { ss ->
                val so = JSONObject()
                so.put("from", ss.from.toString())
                so.put("sessions", ss.sessions)
                sp.put(so)
            }
            o.put("sessionPlan", sp)
            val off = JSONArray()
    ...
    private fun parseStudents(arr: JSONArray): List<Student> {
        ...
        for (i in 0 until arr.length()) {
            ...
            val legacySessions = o.optInt("sessions", 12)
            val spArr = o.optJSONArray("sessionPlan")
            val sessionPlan = ArrayList<SessionSeg>()
            if (spArr != null && spArr.length() > 0) {
                for (k in 0 until spArr.length()) {
                    val so = spArr.optJSONObject(k) ?: continue
                    val f = try { LocalDate.parse(so.optString("from")) } catch (e: Exception) { start }
                    sessionPlan.add(SessionSeg(f, so.optInt("sessions", 12).coerceAtLeast(1)))
                }
            }
            if (sessionPlan.isEmpty()) {
                sessionPlan.add(SessionSeg(start, legacySessions.coerceAtLeast(1)))
            }
            sessionPlan.sortBy { it.from }
